    ...
    if (empty($finalRow)) {
        ...
        if ($finalRow['scouting_team'] === $finalRow['scouted_team']) {
            ...
            if (!empty($finalRow['info_provider'])) {
                ...
                if ($finalRow['scouted_team'] === $teamNumber) {
        ...
        if (!empty($finalRow['wheel_type'])) {
            ...
            if (!empty($finalRow['wheel_num'])) {
                echo ' <strong>(x' . $finalRow['wheel_num'] . ')</strong><br />';
            } else {
                echo '<br />';
            }
        } else {
            if (!empty($finalRow['wheel_num'])) {
                echo 'Wheels: <strong>' . $finalRow['wheel_num'] . '</strong><br />';
            }
        }
...
if ($thingToLoad === "allpit") {
    ...
    while ($finalRow = $response->fetch(PDO::FETCH_ASSOC)) {
        if ($finalRow['scouting_team'] === $teamNumber && $isAdmin) {
            $canDeleteData = true;
        } else {
            $canDeleteData = false;
        }
