...
try {
    $changePassRequest = $db->prepare('UPDATE team_accounts SET team_password=md5(?) WHERE admin_password=md5(?) AND team_number=?');
...
if ($changePassRequest->rowCount() !== 0) {
    echo 'Team password changed successfully.';
} else {
